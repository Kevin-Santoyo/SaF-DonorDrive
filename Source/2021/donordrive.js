...
// Written using DonorDrive API 1.1
...
window.incentivesArray = new Array;
	...
	fetch(requestDonations)
		.then(function (response) {
			if (response.status == 304) {
				...
				response.json().then(data => {
					localStorage.setItem('recentDonation', data[0].donationID);
				});
			}
		}).catch(function (err) {
			console.error(` Err: ${err}`);
		...
		.then(function (response) {
			if (response.status == 304) {
				...
				response.json().then(data => {
					...
					if (data[0].donationID !== recentDonation) {
						for (donor in data) {
	...
	fetch(requestDonations)
		.then(function (response) {
			if (response.status == 304) {
				...
				response.json().then(data => {
					let rowHead = donotable.insertRow();
					rowHead.insertCell(0).textContent = "Name";
					rowHead.insertCell(1).textContent = "Amount";
					rowHead.insertCell(2).textContent = "Message";
					rowHead.insertCell(3).textContent = "Incentive";
					rowHead.insertCell(4).textContent = "Time";
					for (donor in data) {
						var incentivefound = 1;
						let row = donotable.insertRow();
						if (data[donor].displayName) {
							row.insertCell(0).textContent = data[donor].displayName;
						} else {
							row.insertCell(0).textContent = "Anonymous";
						}
						row.insertCell(1).textContent = '$' + data[donor].amount;
						if (data[donor].message) {
							row.insertCell(2).textContent = data[donor].message;
						} else {
							row.insertCell(2).textContent = "No Message";
						}
						for (var i = 0; i < incentivesArray.length; i++) {
							if (incentivesArray[i].includes(data[donor].incentiveID)) {
								incentivefound = 0
								row.insertCell(3).textContent = incentivesArray[i][0];
							}
						}
						if (incentivefound) {
							row.insertCell(3).textContent = "No Incentive"
						}
						row.insertCell(4).textContent = String(new Date(data[donor].createdDateUTC));
					}
					audio.volume = .3
...
function buildIncentiveArray() {

	const requestInfo = new Request(incentivesLink, {
		method: 'GET',
		mode: 'cors',
		cache: 'default',
	});
